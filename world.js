import { csgSolidBake, csgSolidBox, csgSolidLine, csgSolidOpIntersect, csgSolidOpSubtract, csgSolidOpUnion, modelGeoDelete } from "./csg";
import { m4Ident, v3Add, v3AddScale, v3Dot2, v3Length, v3Negate, v3Normalize, v3Sub } from "./types";
// ------------------------------------------------------------------------------------
// Object models
...
export let worldSourceList = [[0, [""]], [0, ["box", "1", "0", "201", "1371", "1376", "2029", "1008", "0", "0", "0", "0"]], [0, ["box", "1", "0", "0", "579", "1", "1", "1", "0", "0", "0", "400"]], [0, ["sub"]], [0, [""]], [0, ["box", "2", "0", "-50", "0", "50", "50", "50", "0", "0", "0", "0"]], [0, ["add"]], [0, [""]], [0, ["castle", "230", "201", "446"]], [0, ["castle", "-230", "201", "446"]], [0, ["castle", "210", "-253", "446"]], [0, ["castle", "-210", "-253", "446"]], [0, [""]], [0, [""]]];
...
let worldGeo;
let worldFn;
export let worldGetGeo = () => worldGeo;
export let worldGetCastles = () => worldCastles;
export let loadLevel = (i) => {
...
export let worldNearestSurfacePoint = (pos) => {
...
export let worldRaycast = (pos, normalizedDir, len) => {
...
export let evaluateNewWorld = (sourceList) => {
...
let evaluateWorldDefSolid = (def) => {
    let result = (name, fn) => [
        fn(...def.slice(1).map((x) => parseInt(x))),
        `${name}(${def.slice(1).map((x) => parseInt(x))})`
    ];
    switch (def[0]) {
...
let worldDefItemIsSolid = (def) => def[0] === 'box' || def[0] === 'line';
let evaluateWorldDefOp = (def, solidA, solidB) => {
    let result = (name, fn) => [
...
let doEvalNewWorld = (worldDef) => {
    worldDef = worldDef.slice();
    modelGeoDelete(worldGeo);
    worldGeo = null;
    worldFn = null;
    let stack = [evaluateWorldDefSolid(worldDef.shift())];
    let castles = [];
    while (worldDef.length > 0) {
        let item = worldDef.shift();
        if (worldDefItemIsSolid(item)) {
            stack.push(evaluateWorldDefSolid(item));
        }
        else if (item[0] === 'castle') {
            castles.push(item.slice(1));
        }
        else {
            let b = stack.pop();
            let a = stack.pop();
            stack.push(evaluateWorldDefOp(item, a, b));
        }
    }
